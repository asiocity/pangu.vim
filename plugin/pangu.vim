...
function! PanGuCore()
    ...
    if &ft != "diff"
        ...
        " 句末加句号
        silent! %s/^\([^\#\-\*\`]\)\(.*\)\([0-9a-zA-Z\u4e00-\u9fa5\u3040-\u30FF]\)$/\1\2\3\./g
...
function! PanGuAddSpaceAtStart()
    ...
    if &ft != "diff"
        ...
        " 句首加空格
        silent! %s/^\([0-9a-zA-Z「\"\u4e00-\u9fa5\u3040-\u30FF]\)/\ \ \ \ \1/g

        call cursor(b:curline, b:curcol)
    endif
endfunction

function! PanGuDelSpaceAtStart()
    let ignore = search("PANGU_DISABLE", 'n')
    if (ignore > 0)
      return
    endif
...
command! -nargs=0 Pangu call PanGuCore()
command! -nargs=0 PanguAddSpaceAtStart call PanGuAddSpaceAtStart()
command! -nargs=0 PanguDelSpaceAtStart call PanGuDelSpaceAtStart()
command! -nargs=0 PanguInsertLine call PanGuInsertLine()
command! -nargs=0 PanguB2G call PanGuB2G()
command! -nargs=0 PanguG2B call PanGuG2B()
